...
const Vote = ({ votingContract, user, userAdd }) => {
  ...
  const giveVote = async (index) => {
    ...
    await toast.promise(
      votingContract.contract.methods.vote(index).send({ from: accounts[0] }),
      {
        pending: "Waiting...",
        success: {
          render({ data }) {
      ...
      <div className="container my-5">
        ...
        {data &&
          (data.length === 0 ? (
            ...
            <>
              <h3 className="text-center text-white fw-bold fs-20 ">Ballot</h3>
              <hr />
                  ...
                  <div className="col-md-4" key={index}>
                    <div className="card">
                      <div className="card-body">
                        <h5 className="card-title">{item.representative}</h5>
                        <hr />
                        <h6>{item.teamName}</h6>
                        <hr />
                        <p className="card-tex ">{item.description}</p>
                        <button
                          disabled={user.voted}
